...
if order<10:
    train_data_file = "SPIRou0"+str(order)+"_train.df" 
else:
    train_data_file = "SPIRou"+str(order)+"_train.df" 
...
for param_idx, (i, snr, nspec) in enumerate(parameters):
    ...
    with h5py.File(filename, "w") as f_out:
        ...
        # -------------------
        obs = Observations(i=0, seed=0, SNR=100, filepath="../data/validation_data/"+val_data_file, N=5,order=args.order,star='barnards')
        ...
        # Create observations
        # -------------------
        obs = Observations(i=i, seed=0, SNR=snr, filepath="../data/validation_data/"+val_data_file, N=nspec,order=args.order,star='barnards')
        ...
        # Load PHOENIX model
        # -------------------
        df = pd.read_pickle("../data/training_data/"+train_data_file)
        a = df[(df['teff']==3200)&(df['logg']==5.0)&(df['metal']==-0.5)]
        phoenix_model = torch.tensor(a['Broadened'].iloc[0]).to(device)
        ...
        group_C.create_dataset("phoenix_model", data=phoenix_model.cpu().numpy())
        ...
        for seed in range(num_seeds):
            ...
            eval_obs = Observations(i=i, seed=seed, SNR=snr, filepath="../data/validation_data/"+val_data_file, N=100,order=args.order,star='barnards')
            ...
            # # Intrinsic RVs
            # sbart_int = RV_Retrieval(snr, eval_obs.original_spectrum, eval_obs.wgrid, eval_obs.inst_wgrid, 0, type='sample')
            # int_rv, int_unc = sbart_int.find_dv(eval_spectra.cpu(), eval_unc.cpu(), eval_obs.berv.cpu(), func='connors')
            # print("Done int",flush=True)

            # bart propr RVs
            phoenix_retrievel = RV_Retrieval(snr, phoenix_model, eval_obs.wgrid, eval_obs.inst_wgrid, 0, type='sample')
            phoenix_rvs, phoenix_unc = phoenix_retrievel.find_dv(eval_spectra.cpu(), eval_unc.cpu(), eval_obs.berv.cpu(), func='connors')
            print("Done phoenix",flush=True)
            ...
            group_seed.create_dataset("phoenix_rv", data=phoenix_rvs)
            group_seed.create_dataset("phoenix_uncertainty", data=phoenix_unc)
