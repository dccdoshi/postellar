...
# -------------------
parameters = list(itertools.product(args.i, args.snr, args.ntemp, [0,1,2,3,4]))
...
for param_idx, (i, snr, nspec, specseed) in enumerate(parameters):
    ...
    filename = f"results/{output_file}_i{i}_snr{snr}nspec{nspec}specseed{specseed}.h5"
    ...
    with h5py.File(filename, "w") as f_out:
        ...
        # Create observations
        # -------------------
        obs = Observations(i=i, seed=specseed, SNR=snr, filepath="../data/validation_data/"+val_data_file, N=nspec,order=args.order)
