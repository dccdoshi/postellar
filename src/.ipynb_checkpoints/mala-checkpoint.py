...
class MALA():
    def __init__(self,obs:torch.Tensor,sig_n:torch.Tensor,berv:torch.Tensor,snr:float,inst_wgrid:torch.Tensor,spec_wgrid:torch.Tensor) -> None:
        ...
        self.start = int(len(self.obs[0,0])*0.1)
        self.end = int(len(self.obs[0,0])*0.9)
    ...
    def find_rv(self,x_init:torch.Tensor,S:torch.Tensor,steps:int):
        ...
        step_size = self.deltaV.value*5#*1/10

        samples = torch.zeros((steps,x_init.shape[0],x_init.shape[1]),dtype=torch.float64).to(DEVICE)
        accepted = torch.zeros((steps,x_init.shape[0],x_init.shape[1]),dtype=torch.float64).to(DEVICE)
        with torch.no_grad():
            sample = x_init.clone().to(DEVICE)
            for j in range(steps):
                sample, accept = self.mala_step(sample, S, step_size)
                accepted[j]=accept
                samples[j]= sample.detach().cpu()
    ...
    def mala_step(self, x:torch.Tensor, S:torch.Tensor, step_size=1e-4, gauss=True,precond_matrix=None, rejection_step=True):
        ...
        if gauss:
            S_nonnorm = S
            score_fn = lambda x: self.score_gaussian(x, S_nonnorm, self.covariance)
            log_prob_fn =  lambda x: self.log_prob_gaussian(x, S_nonnorm, self.covariance)
        ...
        if precond_matrix is not None:
            ...
            dx = step_size * score + np.sqrt(2*step_size) * torch.randn_like(x)
